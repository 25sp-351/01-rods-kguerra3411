...
cutting_option cutting_options[MAX_CUTTING_OPTIONS];
...
void readcutting_options() {
    while (num_cutting_options < MAX_CUTTING_OPTIONS) {
        ...
        int user_input = scanf_s("%d, %d", &cutting_optionLength, &cutting_optionValue);

        if (user_input == EOF) break;  
        if (user_input != 2) {
            fprintf(stderr, "Error: Invalid input format. Expected <length>, <value>.\n");
            exit(1);
        }
        ...
        if (cutting_optionLength > 0 && cutting_optionValue >= 0) {
            ...
            fprintf(stderr, "Error: Invalid cutting_option data. Ensure length > 0 and value >= 0.\n");
            exit(1);
        }
    }
}
